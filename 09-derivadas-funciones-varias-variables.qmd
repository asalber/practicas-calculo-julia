...
a.  ¿En qué dirección debe cambiarse la temperatura y las concentraciones de los reactivos para que la velocidad de reacción aumente lo más rápidamente posible si la temperatura y las concentraciones actuales son the 40ºC, 0.4 mol/l y 0.5 mol/l respectivamente?
    ...
    ∇f = lambdify(ex, (x, y, z))
    sol = ∇f(40, 0.4, 0.5)
    numericq(sol[1], 1e-5, label = "Dirección de cambio de la temperatura: ")
    ```

    ```{julia}
    #| echo: false
    numericq(sol[2], 1e-5, label = "Dirección de cambio del acetato de etilo: ")
    ```

    ```{julia}
    #| echo: false
    numericq(sol[3], 1e-5, label = "Dirección de cambio del hidróxido de sodio: ")
    ```
