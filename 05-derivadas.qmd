...
a.  Calcular la tasa de variación de $y$ con respecto a $x$ en el punto $(1,1)$.
    ...
    @syms u()
    # Reemplazamos y por una función implícita u(x) y calculamos la derivada implícitamente.
    df = diff(f(x, u(x)))
    # Resolvemos la ecuación para la derivada de u(x).
    du = solve(df, diff(u(x)))[1]
...
a.  Dibujar la recta tangente a la curva anterior en el punto $(1,1)$.
    ...
    ```{julia}
    using Latexify
    # Definimos la función de la recta tangente
    tg = 1 + dy(1,1) * (x-1)
    Plots.plot!(tg, label="Tangente "*L"y="*latexify(tg))
...
:::{#exr-taylor-1}
Crear una función para calcular el polinomio de Taylor de grado $n$ una función $f$ en un punto $a$ y utilizarla para dibujar los polinomios de Taylor de la función $f(x)=\operatorname{sen}(x)$ en $a=0$ hasta el grado $5$. Calcular y dibujar el error cometido al aproximar $\operatorname{sen}(\pi/6)$ mediante los polinomios de Taylor hasta el grado $1$ a $9$.
...
Calculamos ahora los errores cometidos al aproximar $\operatorname{sen}(\pi/6)$ mediante los polinomios de Taylor hasta el grado $1$ a $9$.

```{julia}
errores = []
for i in 1:9
    pol = taylor(f(x), 0, i)
    push!(errores, abs(sin(pi/6)-pol(pi/6)))
end
errores
```
...
```{julia}
Plots.scatter(1:9, errores, xlab="Grado", ylab="Error", legend=false)
```
:::
:::
